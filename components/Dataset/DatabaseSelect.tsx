...
export default function DatabaseSelect({
  ...
  useEffect(() => {
    fetch('https://localhost:7219/api/DB')
      .then(async (response) => {
        if (!response.ok) {
  ...
  useEffect(() => {
    if (selectedDatabase) {
      fetch(
        `https://localhost:7219/api/DS/schemas?connectionId=${selectedDatabase}`
      )
        .then(async (response) => {
          if (!response.ok) {
            const errorText = await response.text();
  ...
  useEffect(() => {
    if (selectedDatabase && selectedSchema) {
      fetch(
        `https://localhost:7219/api/DS/tables?connectionId=${selectedDatabase}&schemaName=${selectedSchema}`
      )
        .then(async (response) => {
          if (!response.ok) {
            const errorText = await response.text();
  ...
  useEffect(() => {
    if (selectedDatabase && selectedSchema && selectedTable) {
      fetch(
        `https://localhost:7219/api/DS/fields?connectionId=${selectedDatabase}&schemaName=${selectedSchema}&tableName=${selectedTable}`
      )
        .then(async (response) => {
          if (!response.ok) {
            const errorText = await response.text();
  ...
  const onSubmit = async (data: z.infer<typeof DatasetSchema>) => {
    ...
    try {
      const response = await fetch('https://localhost:7219/api/DS', {
        method: 'POST',
        headers: {
      ...
      router.push(
        `/Projects/${ProjectId}/addChart/${projectName}/${datasetId}`
      );
